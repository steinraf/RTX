...
void sphereLineup(){
    ...
    scene.addShape(std::make_shared<Sphere>(Vector3f{ 0.0, -1000.5, -1.0}, 1000.0, groundMat));
    ...
    for(int i = 0; i < 10; ++i){
        ...
        auto mat = std::make_shared<Metal>(c, 0.3f);

        scene.addShape(std::make_shared<Sphere>(Vector3f{ (i-5)*1.0f,   0, -5},   0.5, mat   ));
    }
    ...
    for(int i = 0; i < 10; ++i){
        ...
        scene.addShape(std::make_shared<Sphere>(Vector3f{ (i-5)*1.0f,   4.0, -5},   0.5, mat   ));
    }

    auto midMat = std::make_shared<Dielectric>(Color{1.0, 1.0, 0.7}, 1.5);
    scene.addShape(std::make_shared<Sphere>(Vector3f{ 0.0,    0.0, -1.0},   0.5, midMat   ));
...
void threeSpheres() {
    ...
    scene.addShape(std::make_shared<Sphere>(Vector3f{-1.0,    0.0, -1.0},   0.5, leftMat  ));
    scene.addShape(std::make_shared<Sphere>(Vector3f{ 0.0,    0.0, -1.0},   0.5, midMat   ));
    scene.addShape(std::make_shared<Sphere>(Vector3f{ 1.0,    0.0, -1.0},   0.5, rightMat ));
...
void randomScene(){
    ...
    scene.addShape(std::make_shared<Sphere>(Vector3f{0, -1000,0},    1000, groundMat));
    ...
    scene.addShape(std::make_shared<Sphere>(Vector3f{0, 1, 0}, 1.0, mat1));

    auto mat2 = std::make_shared<Lambertian>(Color{0.4, 0.2, 0.1});
    scene.addShape(std::make_shared<Sphere>(Vector3f{-4, 1, 0}, 1.0, mat2));

    auto mat3 = std::make_shared<Metal>(Color{0.7, 0.6, 0.5}, 0.0);
    scene.addShape(std::make_shared<Sphere>(Vector3f{4, 1, 0}, 1.0, mat3));
